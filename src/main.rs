...
fn get_opts() -> Opts {
    ...
    let opts = app_from_crate!("\n")
        ...
        .arg(Arg::from_usage(
            "<target/device> 'Target directory for generator or device to operate on'",
        ))
        .arg(
            Arg::from_usage(
                "[additional_targets] 'Unused target directories to satisfy systemd.generator(5)'",
            )
            .number_of_values(2)
